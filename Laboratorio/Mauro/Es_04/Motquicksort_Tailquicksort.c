...
void heapify(int *arr, int n, int root)
{
    ...
    // andiamo a controllare il caso in cui il figlio e' piu' grande della root
    if (left < n && arr[left] > arr[largest])
        largest = left;
    if (right < n && arr[right] > arr[largest])
        largest = right;

    // se c'e' stato effettivamente un aggiornamento
    if (largest != root)
    {
        swap(arr, root, largest); // scambio il tutto con l'aggiornamento della parte
        heapify(arr, n, largest);
    }
}

// heapsort lo useremo per ordinare delle sottoparti dell'array
...
void heap_sort(int *arr, int start, int end)
{
    ...
    for (int i = n - 1; i > 0; i--)
    {
        swap(arr, start, start + i); // inizialmente i=n-1
