...
void init(struct configuration *config) {
    ...
    // lista di proposte.
    int n_algorithms = 2;
    char *algos[] = {"INSERTION", "MERGE"}; //inizializza un array di puntatori a stringhe che a loro volta sono degli array di caratteri
/* praticamente un array di stringhe
per esempio
char *algos = "INSERTION";
*/
 ...
 */
typedef void (*algo_ptr)(int*, int, int); //questa istruzione crea un puntatore ad una funzione. (*alogo_ptr) lo devi interpretare come un enorme asterisco
//il int* indica che gli devo passare un puntatore che punta ad un intero
...
void run_experiments(struct configuration config, FILE *report) {
    ...
    for (int algo_idx=0; algo_idx<config.n_algorithms; algo_idx++) {
        ...
        for (int i=config.min_size; i<=config.max_size; i+=config.step) {
            srand(config.seed);  //srand andrebbe fuori dal for
