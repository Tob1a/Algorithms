...
void push(struct stack *stack, int val)
{
    if (!is_full(stack))
    {
        stack->content[++stack->top] = val; // vuol dire all'interno di stack nella parte contenuto con indice quello dello stack con riferimento al suo top+1
    }
    else
    {
...
int histogram_area(int histogram[], int N)
{
    ...
    while (i < N)
    {
        ...
        {
            int top_index = pop(stack); // questo pop ritorna il contenuto dopo aver eseguito il pop

            int current_area = histogram[top_index] *
                               (is_empty(stack) ? i : i - peek(stack) - 1);
            if (is_empty(stack))
                current_area = histogram[top_index] * i;
            else
                current_area = histogram[top_index] * (i - peek(stack) - 1);
...
void uffici(FILE *in_file, FILE *out_file)
{
    ...
    for (int i = 0; i < N; i++)
    {
        fscanf(in_file, "%d", histogram + i); // histogram+i vuol dire &histogram[i]
    }
